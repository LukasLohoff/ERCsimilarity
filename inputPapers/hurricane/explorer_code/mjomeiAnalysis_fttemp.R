...
#' ---
#' 
#' This is the code used to produce the results and figures seen in "A space-time statistical
#' climate model for hurricane intensification in the North Atlantic basin" by Fraza, Elsner and Jagger.
#' 
#' The code is a bit long as a lot of brute force tactics were used to calculate and acquire the necessary data. But so long as you have this file, along with all the data files, and the necessary packages installed, it should run in no more than a few moments.
#' 
#' A few error messages appear when calculating and placing the climatic variables into their necessary variables. These errors have no effect on the results or the code, and can therefore be ignored.
#' 
#' If, at any point, the following error message is received: "Error in grid.Call(L_textBounds, as.graphicsAnnot(x$label), x$x, x$y,  : 
#'   polygon edge not found", simply re-execute the line of code in question. This occurs when ggplot is not executing properly, and a simple re-run should do the trick.
#' 
